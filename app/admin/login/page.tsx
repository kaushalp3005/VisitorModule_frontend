...
export default function AdminLoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (success) {
        // Check if user is admin email or superuser
        const authData = localStorage.getItem('auth_user');
        if (authData) {
          try {
            const user = JSON.parse(authData);
            // Only allow admin emails or superusers to access admin page
            const canAccessAdmin = user.superuser || (user.admin && isAdminEmail(user.email)) || isAdminEmail(user.email);
            if (canAccessAdmin) {
              router.push('/admin');
              router.refresh();
            } else {
              setError('Access denied. Only admin emails (admin01-05@candorfoods.in) or superusers can access admin page.');
  ...
  return (
    <>
      ...
      />
      <div className="min-h-screen flex items-center justify-center px-4 py-6 md:py-8 bg-gradient-to-br from-orange-50 via-amber-50 to-orange-100">
        <div className="w-full max-w-md">
          <div className="bg-white rounded-2xl shadow-professional-lg border border-orange-200 p-6 sm:p-8 md:p-10">
            <div className="text-center mb-8">
              <h1 className="text-3xl font-bold text-orange-900 mb-2">Admin Login</h1>
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              <div className="space-y-2">
                <Label htmlFor="username" className="text-sm font-semibold text-orange-900">Username</Label>
                <Input
                  id="username"
                  type="text"
                  ...
                  required
                  disabled={isLoading}
                  className="w-full h-12 text-sm border-orange-300 focus:ring-2 focus:ring-orange-500 focus:border-orange-500"
                />
              </div>

              <div className="space-y-2">
                <Label htmlFor="password" className="text-sm font-semibold text-orange-900">Password</Label>
                <div className="relative">
                  <Input
                    ...
                    required
                    disabled={isLoading}
                    className="w-full pr-10 h-12 text-sm border-orange-300 focus:ring-2 focus:ring-orange-500 focus:border-orange-500"
                  />
                  <button
                    type="button"
                    onClick={() => setShowPassword(!showPassword)}
                    className="absolute right-3 top-1/2 -translate-y-1/2 text-orange-400 hover:text-orange-600 touch-manipulation"
                    tabIndex={-1}
                    disabled={isLoading}
                  >
                    {showPassword ? (
              ...
              <Button
                type="submit"
                className="w-full h-12 text-base font-semibold bg-orange-600 text-white hover:bg-orange-700 shadow-md hover:shadow-lg transition-all"
                disabled={isLoading}
              >
            ...
            <Button
              type="button"
              variant="ghost"
              className="w-full text-sm text-orange-600 hover:text-orange-900 h-10 mt-4"
              onClick={() => setShowForgetPassword(true)}
            >
              Forget Password
            </Button>

            <div className="text-center mt-6 pt-6 border-t border-orange-200">
        ...
        <DialogContent className="sm:max-w-md max-w-[calc(100vw-2rem)] mx-4">
          ...
          >
            ...
            <div className="flex gap-2 md:gap-3">
              ...
              <Button
                type="submit"
                className="flex-1 h-10 md:h-11 text-sm md:text-base touch-manipulation bg-orange-600 hover:bg-orange-700"
                disabled={isChangingPassword}
              >
